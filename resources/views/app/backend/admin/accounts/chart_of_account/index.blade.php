...
@push('title', translate('Add chart of accounts'))
    ...
    <div class="card">
        <div class="card-header bg-white border-color p-10 border-bottom-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <p class="fs-16 fw-500 text-secondary mb-0">{{ translate('All  Chart Of Accounts') }}</p>
                ...
                <div class="chart-control d-flex align-items-center flex-wrap gap-8">
                    ...
                    <x-btn-canvas :title="translate('Add chart of accounts')" :url="path(['backend::admin.accounts.chart_of_account.create'])" />
                </div>
            </div>
        </div>
                    ...
                    @foreach ($chartofaccounts as $key => $chartofaccount)
                        <tr>
                            <td class="d-flex align-items-center gap-2">
                                <input type="checkbox" id="row{{ $key }}" class="form-check-input rowCheckbox mt-0">
                                <label for="row{{ $key }}" class="form-check-label fs-12 mb-0">{{ $key + 1 }}</label>
                            </td>
                            <td>{{ $chartofaccount->account_name }}</td>
                            <td>{{ $chartofaccount->account_type }}</td>
                            <td>{{ $chartofaccount->notes }}</td>
                            <td>
                                <x-dropdown>
                                    <x-drop-canvas :title="translate('Edit')" :url="path(['backend::admin.accounts.chart_of_account.edit', 'id' => $chartofaccount->id])" :icon="'fi fi-rr-file-edit'" divider />
                                    <x-drop-delete :url="route('chartofaccounts.delete', $chartofaccount->id)" />
                                </x-dropdown>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="pagination-wraper p-12 d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="showing-pagination d-flex align-items-center gap-8">
                    <span class="fs-12">{{ translate('Showing') }}
                        {{ $chartofaccounts->firstItem() }}–{{ $chartofaccounts->lastItem() }} {{ translate('of') }}
                        {{ $chartofaccounts->total() }}</span>
                </div>
