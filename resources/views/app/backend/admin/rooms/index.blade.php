    ...
    <div class="card">
        <div class="card-header bg-white border-color p-10 border-bottom-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <p class="fs-16 fw-500 text-secondary mb-0">{{ translate('All Rooms') }}</p>
                ...
                <div class="chart-control d-flex align-items-center flex-wrap gap-8">
                    ...
                    <x-btn-canvas :title="translate('Add Subject')" :url="path(['backend::admin.rooms.create'])" />
                </div>
            </div>
        </div>
            ...
            <table
                ...
                <thead class="table-light">
                    <tr class="fs-12 fw-400 text-uppercase">
                        ...
                        <th scope="col">{{ translate('Room Name') }}</th>
                ...
                <tbody>
                    @foreach ($rooms as $key => $room)
                        <tr>
                            ...
                            <td>
                                <x-dropdown>
                                    <x-drop-canvas :title="translate('Edit')" :url="path(['backend::admin.rooms.edit', 'id' => $room->id])" :icon="'fi fi-rr-file-edit'" divider />
                                    <x-drop-delete :url="route('rooms.delete', $room->id)" />
                                </x-dropdown>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="pagination-wraper p-12 d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="showing-pagination d-flex align-items-center gap-8">
                    <span class="fs-12">{{ translate('Showing') }}
                        {{ $rooms->firstItem() }}–{{ $rooms->lastItem() }} {{ translate('of') }}
                        {{ $rooms->total() }}</span>
                </div>
