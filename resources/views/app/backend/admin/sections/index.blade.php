    ...
    <div class="card">
        <div class="card-header bg-white border-color p-10 border-bottom-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <p class="fs-16 fw-500 text-secondary mb-0">{{ translate('All Sections') }}</p>
                ...
                <div class="chart-control d-flex align-items-center flex-wrap gap-8">
                    ...
                    <x-btn-canvas :title="translate('Add Sections')" :url="path(['backend::admin.sections.create'])" />
                </div>
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($sections as $section)
                        <tr>
                            ...
                            <td>{{ $section->name }}</td>
                            <td>{{ $section->classes->name ?? 'N/A' }}</td>
                            <td>
                                <x-dropdown>
                                    <x-drop-canvas :title="translate('Edit')" :url="path(['backend::admin.sections.edit', 'id' => $section->id])" :icon="'fi fi-rr-file-edit'" divider />
                                    <x-drop-delete :url="route('sections.delete', $section->id)" />
                                </x-dropdown>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="pagination-wraper p-12 d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="showing-pagination d-flex align-items-center gap-8">
                    <span class="fs-12">{{ translate('Showing') }}
                        {{ $sections->firstItem() }}–{{ $sections->lastItem() }} {{ translate('of') }}
                        {{ $sections->total() }}</span>
                </div>
