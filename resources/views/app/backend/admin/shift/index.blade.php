    ...
    <div class="card">
        <div class="card-header bg-white border-color p-10 border-bottom-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <p class="fs-16 fw-500 text-secondary mb-0">{{ translate('All Shifts') }}</p>
                ...
                <div class="chart-control d-flex align-items-center flex-wrap gap-8">
                    ...
                    <x-btn-canvas :title="translate('Add Shift')" :url="path(['backend::admin.shift.create'])" />
                </div>
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($shifts as $key => $shift)
                        <tr>
                            ...
                            <td>
                                <x-dropdown>
                                    <x-drop-canvas :title="translate('Edit')" :url="path(['backend::admin.shift.edit', 'id' => $shift->id])" :icon="'fi fi-rr-file-edit'" divider />
                                    <x-drop-delete :url="route('shifts.delete', $shift->id)" />
                                </x-dropdown>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="pagination-wraper p-12 d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="showing-pagination d-flex align-items-center gap-8">
                    <span class="fs-12">{{ translate('Showing') }}
                        {{ $shifts->firstItem() }}–{{ $shifts->lastItem() }} {{ translate('of') }}
                        {{ $shifts->total() }}</span>
                </div>
