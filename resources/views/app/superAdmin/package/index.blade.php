    ...
    <div class="card">
        <div class="card-header bg-white border-color p-10 border-bottom-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <p class="fs-16 fw-500 text-secondary mb-0">{{ translate('All Packages') }}</p>
                ...
                <div class="chart-control d-flex align-items-center flex-wrap gap-8">
                    ...
                    <x-btn-canvas :title="translate('Add Package')" :url="path(['app.superAdmin.package.create'])" />
                </div>
            </div>
        </div>
            ...
            <table
                ...
                <thead class="table-light">
                    <tr class="fs-12 fw-400 text-uppercase">
                        ...
                        <th scope="col">{{ translate('Subtitle') }}</th>
                ...
                <tbody>
                    @foreach ($packages as $key => $package)
                        <tr>
                            ...
                            <td>{{ $package->title ?? translate('N/A') }}</td>
                            <td>{{ $package->sub_title ?? translate('N/A') }}</td>
                            <td>{{ $package->description ?? translate('N/A') }}</td>
                            <td>${{ $package->price ?? 0 }}</td>
                            <td>{{ $package->discount ?? 0 }}%</td>
                            <td>{{ $package->type ?? translate('N/A') }}</td>
                            <td>{{ $package->order }}</td>
                            <td>
                                @if ($package->icon)
                                    <img src="{{ $package->icon }}" width="30" height="30"
                                        alt="{{ translate('icon') }}">
                                @else
                            ...
                            <td>
                                @if ($package->status == 1)
                                    <span
                                        class="badge badge-complete fs-10 fw-500 rounded-4">{{ translate('Complete') }}</span>
                                @else
                                    <span
                                        class="badge badge-cancel fs-10 fw-500 rounded-4">{{ translate('Incomplete') }}</span>
                                @endif
                            </td>
                            <td>
                                <x-dropdown>
                                    <x-drop-canvas :title="translate('Add Feature')" :url="path(['app.superAdmin.packageFeatures.create', 'id' => $package->id])" :icon="'fi fi-rr-magic-wand'" />

                                    <x-drop-canvas :title="translate('Edit')" :url="path(['app.superAdmin.package.edit', 'id' => $package->id])" :icon="'fi fi-rr-file-edit'" divider />
                                    <x-drop-delete :url="route('package.delete', $package->id)" />
                                </x-dropdown>
                            </td>
                        </tr>
                    @endforeach
