    ...
    <div class="card">
        <div class="card-header bg-white border-color p-10 border-bottom-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <p class="fs-16 fw-500 text-secondary mb-0">{{ translate('All Customers') }}</p>
                ...
                <div class="chart-control d-flex align-items-center flex-wrap gap-8">
                    ...
                    <x-btn-canvas :title="translate('Add Customer')" :url="path(['app.superAdmin.customers.create'])" />
                </div>
            </div>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-8">
                                    <figure>
                                        <img src="{{ $customer->photo }}" class="rounded-circle img-fluid" width="45"
                                            height="45" alt="customer">
                                    </figure>
                                    <div class="form-check-label fs-12">
                                        <span class="d-block fw-500 mb-6">{{ $customer->name ?? 'N/A' }}</span>
                                        <span class="text-body">{{ $customer->email ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <x-dropdown>
                                    <x-drop-canvas :title="translate('Edit')" :url="path(['app.superAdmin.customers.edit', 'id' => $customer->id])" :icon="'fi fi-rr-file-edit'" divider />
                                    <x-drop-delete :url="route('customers.delete', $customer->id)" />
                                </x-dropdown>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="pagination-wraper p-12 d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="showing-pagination d-flex align-items-center gap-8">
                    <span class="fs-12">{{ translate('Showing') }}
                        {{ $customers->firstItem() }}–{{ $customers->lastItem() }} {{ translate('of') }}
